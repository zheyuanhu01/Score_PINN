...
class MLP(hk.Module):
    ...
    def __call__(self, x, t):
        ...
        d = args.dim
        X = X * t - args.cauchy_gamma *x / (args.cauchy_gamma**2 / (d + 1) + jnp.mean(x**2) * (1 - 1 / (d + 1)))
        return X
...
class MLP_Q(hk.Module):
    ...
    def __call__(self, x, t):
        ...
        for dim in self.layers[:-1]:
            ...
            X = jnp.tanh(X)
        X = hk.Linear(self.layers[-1])(X)
        X = X[0] * t + constant - (args.dim + 1) / (2 * args.dim) * jnp.log(args.cauchy_gamma**2 + jnp.sum(x**2))
        return X

X = np.loadtxt("BC_Data_X_" + str(args.dim) + ".txt")
T = np.loadtxt("BC_Data_T_" + str(args.dim) + ".txt")
Q = np.loadtxt("BC_Data_Q_" + str(args.dim) + ".txt")
...
class PINN:
    ...
    def resample(self, N):
        mean, cov = np.zeros(self.dim), np.eye(self.dim)
        cauchy = multivariate_t(loc=mean, shape=cov, df=1)
        x0 = cauchy.rvs(size=N) * args.cauchy_gamma
        tf = np.random.rand(N, ) + 1e-2
        xf = np.random.randn(N, self.dim) * np.sqrt(Gamma).reshape(1, self.dim) * \
            np.sqrt(tf).reshape(-1, 1) + x0
        return x0, xf, tf
    ...
    def step(self, params, opt_state, rng, x0, xf, tf):
    ...
    def train_score(self):
        ...
        for n in tqdm(range(self.epoch)):
            x0, xf, tf = self.resample(self.N_SM)
            current_loss, self.params, self.opt_state, self.rng = self.step(self.params, self.opt_state, self.rng, x0, xf, tf)
